...
# CHUNK_SIZE = 5000
CHECK_POINT_FILE_NAME = 'checkpoint.{epoch:02d}.hdf5'
CENSUS_MODEL = '1smodel.hdf5'
...
class ContinuousEval(keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        if epoch > 0 and epoch % self.eval_frequency == 0 and len(checkpoints) > 1:
            ...
            test_data = model.generator_input(
                self.eval_files, chunk_size=CHUNK_SIZE)
            # print test_data.shape
            loss, acc = census_model.evaluate_generator(test_data,
                                                        steps=self.steps)
            print '\nEvaluation epoch[{}] metrics[{:.2f}, {:.2f}] {}'.format(
                epoch, loss, acc, census_model.metrics_names)


def dispatch(ticker_list,
             sample_dir,
             job_dir,
             train_steps,
             learning_rate,
    ...
    data_set = read_shuffle_data.read_data(
        ticker_list,
        sample_path=sample_dir,
        test_points=eval_points)
    print "Test", data_set.test_data.shape, "Train", data_set.train_data.shape
    data_set.print_stat()
    data_set.save_whole(job_dir)

    #  s_model = model.model_create(learning_rate)
#
    #  try:
    #  os.makedirs(job_dir)
    #  except:
...
#
    ...
    #  os.path.join(job_dir, CHECK_POINT_FILE_NAME),
    #  monitor='val_loss',
    #  verbose=1,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ticker-list',
                        required=True,
                        type=str,
                        help='ticker list p file local or GCS')
    parser.add_argument('--sample-dir',
                        required=True,
                        type=str,
                        help='Sample files dir local or GCS')
    ...
    parser.add_argument('--train-steps',
                        type=int,
                        default=100,
                        help="number of training steps to perform per Epoch")
    ...
    parser.add_argument('--eval-points',
                        type=int,
                        default=2000,
                        help='Data points used as test points.')
