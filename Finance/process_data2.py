...
class process_data:
    def __init__(self, filename):
        df_1D = pickle.load(open(filename, "rb"))
    ...
    def sampling(self, sample_distance=100, sample_len=260):
        ...
        N_of_samples = int(sample_range / sample_distance)
        ...
        for i in range(N_of_samples):
            ...
            if i == 0:
                all_samples = unit_sample
            else:
                all_samples = np.concatenate(
                    (all_samples, unit_sample), axis=0)
...
def main():
    data_cooked = process_data('MMM.p')
    ...
    #  print samples[i, 0, 0],
    #  print ""
    with bz2.BZ2File("MMM_samples.p.bz2", "w") as pfile:
