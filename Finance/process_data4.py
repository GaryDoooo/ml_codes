...
class process_data:
    ...
    def sampling(self, sample_distance=100, sample_len=260):
        ...
        for i in range(N_of_samples):
            while True:
                ...
                if self.data_check_OK(np.concatenate((sample1D, sample5D, sample10D), axis=0)):
                    break
            # 4 is adj close
    ...
    def data_check_OK(self, data):
        if 0 < np.sum(1 * np.isnan(data)):
            print "found nan"
            return False
        elif np.max(np.abs(data[:, 1:5])) > 10:
            print "found 10x ohlc change"
            return False
        elif np.max(np.abs(data[:, 6:11])) > 5:
            print "found 5x ma change"
            return False
        elif np.max(np.abs(data[:, 11:14])) > 100:
            print "found macd > 100"
            return False
        elif np.max(np.abs(data[:, 5])) > 100:
            print "found volume 100x change."
            return False
        else:
            return True
...
def main():
    ...
    data_set = np.zeros((0, 0, 0))
    for ticker in total_list:
        samples = process_stock("data/" + ticker)
        if samples.shape[1] > 1:
            if data_set.shape[0] < 1:
                data_set = samples
            else:
                data_set = np.concatenate((data_set, samples), axis=0)
    print "data set size", data_set.shape
    #  with bz2.BZ2File("wholedata.p.bz2", "w") as pfile_handle:
    pickle.dump(data_set, open("samples/wholedata.p", "wb"),
                protocol=pickle.HIGHEST_PROTOCOL)
