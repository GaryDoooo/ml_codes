...
class read_data:
    ...
    def print_stat(self, input_data):
        for i in range(input_data.shape[-1]):
            print "Row", i,
            print "Avg=%.2f" % np.mean(input_data[:, :, i]),
            print "Std=%.2f" % np.std(input_data[:, :, i]),
            print "Max=%.2f" % np.max(input_data[:, :, i]),
            print "Min=%.2f" % np.min(input_data[:, :, i]),
...
def main():
    data_set = read_data("wholedata.p.bz2")
