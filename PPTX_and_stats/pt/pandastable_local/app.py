...
class DataExplore(Frame):
    ...
    def loadMeta(self, table, meta):
        ...
        if isinstance(table.multiplecollist, tuple):
    ...
    def saveMeta(self, table):
        ...
        #print (meta['plotviewer'])
        # save row colors since its a dataframe and isn't picked up by
        # getattributes currently
        meta['table']['rowcolors'] = table.rowcolors
        # save child table if present
        if table.child is not None:
    ...
    def loadProject(self, filename=None, asksave=False):
        ...
        if filename is None:
            filename = filedialog.askopenfilename(
                defaultextension='.dexpl"', initialdir=self.defaultsavedir, filetypes=[
                    ("project", "*.dexpl"), ("All files", "*.*")], parent=self.main)
        if not filename:
            return
    ...
    def addSheet(self, sheetname=None, df=None, meta=None, select=False):
        ...
        def checkName(name):
            ...
            if name in names:
                messagebox.showwarning(
                    "Name exists", "Sheet name already exists!")
                return 0
        ...
        if table.plotted == 'main':
            table.plotSelected()
        elif table.plotted == 'child' and table.child is not None:
