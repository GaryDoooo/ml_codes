...
class Table(Canvas):
    ...
    def setFont(self):
        ...
        if isinstance(self.fontsize, str):
        ...
        if hl is not None:
            for col in self.visiblecols:
                ...
                for row in rows:
                    ...
                    if val:
                        self.drawRect(
                            row, col, color='lightblue', tag='temprect', delete=1)
        return
            ...
            if colname in list(rc.columns):
                ...
                for row in rows:
                    ...
                    if not pd.isnull(clr):
                        self.drawRect(
                            row, col, color=clr, tag='colorrect', delete=0)
        return
    ...
    def resetIndex(self, ask=True, drop=False):
        ...
        self.storeCurrent()
        df = self.model.df
        if (df.index.name is None or df.index.names[0] is None) and ask:
    ...
    def findDuplicates(self):
        ...
        keep = ['first', 'last']
        d = MultipleValDialog(
            title='Find duplicates', initialvalues=[
                False, False, keep], labels=[
                'Remove duplicates:', 'Use selected columns:', 'Keep:'], types=[
                'checkbutton', 'checkbutton', 'combobox'], parent=self.parentframe)
        if d.result is None:
            return
        remove = d.results[0]
    ...
    def cleanData(self):
        ...
        c = len(df.columns)
        n = messagebox.askyesno(
            "Clean Data?", "New table has %s rows and %s columns. Proceed?" %
            (r, c), parent=self.parentframe)
        if n is None:
            return
        self.model.df = df
        self.redraw()
    ...
    def applyTransformFunction(self, evt=None):
        ...
        d = MultipleValDialog(
            ...
            tooltips=(
                None,
                'Summary function for windowing',
                'Window type',
                'Window size',
                'Suffix for new column',
                'Replace column'),
            parent=self.parentframe)
        if d.result is None:
            return
    ...
    def resample(self):
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            messagebox.showwarning(
                "No datetime index",
                'Your date/time column should be the index.',
                parent=self.parentframe)
            return
    ...
    def statsViewer(self):
        ...
        if not hasattr(self, 'sv') or self.sv is None:
    ...
    def functionsBar(self, evt=None):
        ...
        if hasattr(self, 'funcsframe') and self.funcsframe is not None:
            return
    ...
    def evalBar(self, evt=None):
        ...
        if hasattr(self, 'evalframe') and self.evalframe is not None:
            return
        if not hasattr(self, 'formulae'):
            self.formulae = {}
        ...
        Checkbutton(
            bf,
            text="Update plot",
            variable=self.updateplotvar).pack(
            side=LEFT)
        Checkbutton(
            bf,
            text="Place new columns",
            variable=self.placecolvar).pack(
            side=LEFT)
        Checkbutton(
            bf,
            text="Recalculate all",
            variable=self.recalculatevar).pack(
            side=LEFT)
        return
    ...
    def get_col_clicked(self, event):
        ...
        for colpos in self.col_positions:
            try:
                nextpos = self.col_positions[self.col_positions.index(
                    colpos) + 1]
            except BaseException:
                nextpos = self.tablewidth
            if x > colpos and x <= nextpos:
                # print 'x=', x, 'colpos', colpos,
                # self.col_positio.drawSelectedRectns.index(colpos)
    ...
    def handle_mouse_drag(self, event):
        ...
        if self.endrow != self.startrow:
            if self.endrow < self.startrow:
                self.multiplerowlist = list(
                    range(self.endrow, self.startrow + 1))
            else:
                self.multiplerowlist = list(
                    range(self.startrow, self.endrow + 1))
            self.drawMultipleRows(self.multiplerowlist)
            self.rowheader.drawSelectedRows(self.multiplerowlist)
    ...
    def handle_right_click(self, event):
        ...
        if (rowclicked in self.multiplerowlist or self.allrows) and colclicked in self.multiplecollist:
            self.rightmenu = self.popupMenu(
                event, rows=self.multiplerowlist, cols=self.multiplecollist)
        else:
            if 0 <= rowclicked < self.rows and 0 <= colclicked < self.cols:
                self.clearSelected()
                self.allrows = False
    ...
    def transform(self):
        ...
        d = MultipleValDialog(
            ...
            tooltips=(
                None,
                'value to apply with arithmetic operations',
        ...
        if funcname in ['subtract', 'divide', 'mod', 'power', 'round']:
            if use_sel:
                df.ix[rows, cols] = df.ix[rows, cols].applymap(
                    lambda x: func(x, const))
            else:
    ...
    def doCombine(self):
        ...
        if self.child is None:
            messagebox.showwarning(
                "No data",
                'You need a sub-table to merge with.',
                parent=self.parentframe)
            return
        self.storeCurrent()
    ...
    def convertNumeric(self):
        ...
        for c in colnames:
            ...
            self.model.df[c] = pd.to_numeric(
                x, errors='coerce').astype(convtype)
    ...
    def plotSelected(self):
        ...
        if not hasattr(self, 'pf') or self.pf is None:
            self.pf = PlotViewer(table=self)
        else:
            if isinstance(self.pf.main, Toplevel):
    ...
    def drawMultipleCells(self):
        ...
        c, d, x2, y2 = self.getCellCoords(
            rows[len(rows) - 1], cols[len(cols) - 1])
    ...
    def saveAs(self, filename=None):
        ...
        if filename is None:
            filename = filedialog.asksaveasfilename(
                parent=self.master, initialdir=self.currentdir, filetypes=[
                    ("pickle", "*.pickle"), ("All files", "*.*")])
        if filename:
...
class ToolBar(Frame):
    ...
    def __init__(self, parent=None, parentapp=None):
        ...
        def func(): return self.parentapp.importCSV(dialog=1)
        addButton(self, 'Import', func, img, 'import csv')
        img = images.excel()
        addButton(
            self,
            'Load excel',
            self.parentapp.loadExcel,
            img,
            'load excel file')
        img = images.copy()
        addButton(
        ...
        addButton(
            ...
            self.parentapp.tableFromSelection,
            img,
            'sub-table from selection')
...
class statusBar(Frame):
    ...
    def __init__(self, parent=None, parentapp=None):
        ...
        addButton(
            fr,
            'Contract Cols',
            self.parentapp.contractColumns,
            img,
            'contract columns',
            side=LEFT,
            padding=1)
