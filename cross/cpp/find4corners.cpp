...
#define CENTER_AVG_SIZE_RATIO 20
...
vector<point> get_EWNS_from_ctr(const vector<int>& im, const int width,
                                const int height) { //,
    ...
    long long sum_lum = 0;
    int cnt = 0;
    for (int x = x0 - width / CENTER_AVG_SIZE_RATIO;
         x <= x0 + width / CENTER_AVG_SIZE_RATIO; x++)
        for (int y = y0 - height / CENTER_AVG_SIZE_RATIO;
             y <= y0 + height / CENTER_AVG_SIZE_RATIO; y++) {
            sum_lum += im[y * width + x];
            cnt++;
        }

    int thd = int(sum_lum / cnt / 10);

    // int thd = im[x0 + y0 * width] / 10;
    // auto maxElementIterator = max_element(im.begin(), im.end());
    // int thd = (*maxElementIterator) / 10;
    cout << "Threshold is " << thd << endl;
    // east = west = x0;
    // north = south = y0;
    stk.push({x0, y0});
    vis[y0][x0] = 1;
    ...
    while (!stk.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (new_x < 0 or new_y < 0 or new_x >= width or new_y >= height)
                continue;
            if (im[new_y * width + new_x] < thd)
                continue;
            if (ManhattanDis(new_x, new_y, x0, y0) > mdis)
                apex = 0;
            if (vis[new_y][new_x])
                continue;
            vis[new_y][new_x] = 1;
...
vector<point> find4corners(vector<int>& imageData, int width, int height) {
    ...
    cout << "Get image H " << height << " W " << width << endl;
