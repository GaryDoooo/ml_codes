...
def trim_df_by_ticker(df, list1, list2=[]):
...
class data_gen:
    def __init__(self,
        ...
        (av_data_df, unfound_list_av, self.av_tickers,
         self.av_col_per_ticker) = trim_df_by_ticker(av_data_df,
                                                     predict_ticker_list,
                                                     input_ticker_list)
        (fred_data_df, unfound_list_fred, self.fred_tickers,
         self.fred_col_per_ticker) = trim_df_by_ticker(fred_data_df,
                                                       predict_ticker_list,
                                                       input_ticker_list)
        self.unfound_list = [
        ...
        (self.input_df, _, _, _) = trim_df_by_ticker(
            full_df, self.input_ticker_list)
        (self.predict_df, _, _, _) = trim_df_by_ticker(
            full_df, self.predict_ticker_list)
        self.input_col_list = list(self.input_df.columns)
    ...
    def gen_data_sets(self,
        ...
        index = self.predict_df.index.values
        x = None
        y = None
        for i in range(number_of_data_sets):
            today = today_list[i]
            input_array = self.input_df.loc[index[today +
                                                  1-input_dist]:index[today], :].values
            predict_array = []
            for dist in predict_dist_list:
                for ticker in self.predict_ticker_list:
                    predict_array.append(accumulated_change(
                        ticker, self.predict_df, index[today], index[today+dist]))
                    # predict array is 1D with [[ticker1,ticker2...],[dist2 set],[dist3 set]...]
            predict_array = np.expand_dims(np.asarray(predict_array), axis=0)
            input_array = np.expand_dims(np.asarray(input_array), axis=0)
            if x is None:
                x = input_array
            else:
                x = np.append(x, input_array, axis=0)
            if y is None:
                y = predict_array
            else:
                y = np.append(y, predict_array, axis=0)
        return x, y

    def pick_input_on_date(self,
                           input_dist,  # how many days to look back, this is an int
                           date_list):  # specify date type for indexing the df directly
        today_list = []  # this is a list of int index number
        index = self.predict_df.index.values
        for day in date_list:
            today_list.append(list(index).index(day))
        print(today_list)
        x = None
...
def date_index(year, mon, day):
    #    return pd.DatetimeIndex(["%4d-%02d-%02d" % (year, mon, day)])[0]
    return np.datetime64(datetime.datetime(year, mon, day, 0, 0, 0, 0))


def test_read():
    d = read_data("test_data_gen")
...
def main():
    ...
    print("lens of input cols ", len(d.input_col_list))
    print(d.input_col_list)
    test_read()
    x = d.pick_input_on_date(50, [date_index(2018, 12, 3)])
    print(x.shape)
