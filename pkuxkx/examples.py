...
# Trigger、Alias仅有成功，即仅onSuccess方法会被调用，该方法参数参考了MushClient，会传递name, line,
# wildcards三个参数，含义与MushClient相同；
...
class Configuration:
    ...
    def _initTriggers(self):
        ...
        # raw        :
        # 原始匹配方式，默认False。原始匹配方式下，不对VT100下的ANSI颜色进行解码，因此可以匹配颜色；正常匹配仅匹配文本
    ...
    def _initCommands(self):
        ...
        # 3. retry_tri:
        # 需要重新尝试命令的触发器，可以为空（仍以移动为例，当向某个方向移动，出现“你现在正忙着呢”之后，可以在等待2s之后，再次尝试该命令，知道到达最大尝试次数
    ...
    def oncmd_hpbrief(self, name, cmd, line, wildcards):
        ...
        line1 = "【精神】 {0:<8} [{5:3.0f}%] / {1:<8} [{2:3.0f}%]  |【精力】 {3:<8} / {4:<8} [{6:3.0f}%]".format(
            ...
            float(
                var1[4]))
        var2 = self.session.getVariables(
            ("qi", "effqi", "maxqi", "neili", "maxneili"))
